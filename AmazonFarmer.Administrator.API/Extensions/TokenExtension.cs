    ...
    public class JWTService
    {
        ...
        public static TokenResponse GenerateJwt(UserDTO user, string secret, string issuer, int expirationInDays, IEnumerable<Claim> pClaims)
        {
            var claims = new List<Claim>
            {
                ...
                new Claim("expOn", ((long)DateTime.UtcNow.AddSeconds(expirationInDays).ToUniversalTime().Subtract(new DateTime(1970, 1, 1)).TotalSeconds).ToString())
            ...
            DateTime expiresIn = DateTime.UtcNow.AddSeconds(expirationInDays);
